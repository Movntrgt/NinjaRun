...
public class Player1Controller : MonoBehaviour
{
    ...
    {
        inputDirection.x = Input.GetAxis("Player1Horizontal");
        CheckForWall();
        CheckForGround();
        Running();
    ...
    {
        // Calculate direction and magnitude of intended velocity.
    ...
    {
        if (Input.GetKeyDown(KeyCode.UpArrow) && isGrounded == true) // If the jump key is pressed and the player is grounded
        {
            rb.AddForce(Vector2.up * jumpForce, ForceMode2D.Impulse); // Vertical upwards force is applied to the player
        }

        if (Input.GetKeyDown(KeyCode.DownArrow) && isGrounded == false) // If the down arrow key is pressed and the player is not on the ground
        {
    ...
    private void WallJump()
    {
        ...
        if (Input.GetKeyDown(KeyCode.UpArrow) && wallJumpCount > 0) // If the jump key is pressed and the player can still jump
        {
